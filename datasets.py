...
class SeismicDatasetLSTM(Dataset):
      ...
      def __getitem__(self, index):
        trace_index = self.trace_indices[index]
        x = torch.tensor(self.seismic[trace_index][np.newaxis, :].T, dtype=torch.float).to(torch.device('cuda' if torch.cuda.is_available() else 'cpu'))
        y = torch.tensor(self.model[trace_index][np.newaxis, :].T, dtype=torch.float).to(torch.device('cuda' if torch.cuda.is_available() else 'cpu'))
        return x, y
